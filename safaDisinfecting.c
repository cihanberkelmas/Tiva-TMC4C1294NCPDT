...
#define NTP_IP            "132.163.97.3"
#define SOCKETTEST_IP     "192.168.1.29"
...
Void taskLEDWrite(UArg arg1, UArg arg2)
{
    ...
    while(1) {
        ...
        if(total < 1000){
            // total => ADC value average is less then 1000
            detector = 1;
            // PF1--> 1 LED is Off
...
Void functionTimer(UArg arg1, UArg arg2) // getting TIME values
{
    ...
    while(1){
        ...
        inet_pton(AF_INET, "132.163.97.3", &(serverAddr.sin_addr));
...
Void taskHercules(UArg arg1, UArg arg2)
{
    while(1){
        ...
        if(data == 0){
            ...
            // detector --> 0 led is on
            strcat(message,buffer);
            strcat(message," There is people or animal in area. UVC LED is ON\n");

        }
        else{
            //otherwise --> led is off
            strcat(message,buffer);
            strcat(message," No alive detected. UVC LEDS are OFF");
            strcat(message,"\n");
        }
